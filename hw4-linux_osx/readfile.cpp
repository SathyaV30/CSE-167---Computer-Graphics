...
void readfile(const char* filename) 
{
  ...
  if (in.is_open()) {
      ...
      if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
        ...
        if (cmd == "light") {
            ...
            if (validinput) {
              ...
              // Those uarrays can then be used in display too.  
            int start = numused * 4;
            for (int i = 0; i < 4; i++) {
                lightposn[start + i] = values[i];
                lightcolor[start + i] = values[4 + i];
            }
              ++numused; 
            }
    ...
    if (validinput) {
        ...
        for (int i = 0; i < 3; i++) {
            lightposn[start + i] = values[i];
            lightcolor[start + i] = values[i + 3];
        }
        lightposn[start + 3] = 0; 
        lightcolor[start + 3] = 1; 
        numused++;
    }
}
else if (cmd == "point") {
    ...
    if (validinput) {
          ...
          if (validinput) maxdepth = values[0];       
            ...
            if (cmd == "sphere") {
              ...
              if (validinput){
                  ...
                  for (int i = 0; i < 4; i++) {
                    if (i == 3) {
                      //size corresponds to radius, values[0],values[1],values[2] correspond to center point
                      obj->size = values[i];
                      obj->coords[i] = 0;
                    } else {
                    obj->coords[i] = values[i];
                    }
                  }
              }
            } else {
        ...
        // Also look at demo.txt to get a sense of why things are done this way.
          ...
          if (validinput) {
            ...
            mat3 rotMat3 = Transform::rotate(values[3], axis);
            mat4 rotMat4 = mat4(rotMat3);
